...
interface ShareModalProps {
   ...
   id: string | null;
   onClose: () => void;
   onSend: (id: string, recipientIds: string[], message?: string) => Promise<void>;
   description: string;
}

export default function ShareModal({ isOpen, id, onClose, onSend, description }: ShareModalProps) {
   ...
   async function handleSend() {
      if (!hasSelection || sending || !id) return;
      setSending(true);
      try {
         await onSend(id, [...selectedIds], message.trim() || undefined);
         toast('Sent.');
         handleClose();
      } catch (e) {
